...
contract MultiSig {
    ...
    mapping(address =>mapping(uint8 => bool)) hasApproved;
    ...
    function updateQuorum(uint8 newQuorum, address[] memory _validSigners) external {
        require(newQuorum > 1, "Too small");
        require(newQuorum != quorum, "No change made" );
        require(isValidSigner[msg.sender], "Invalid Signer");
        require(_validSigners.length > 0, "Few signers");
        

        for(uint256 i = 0; i< _validSigners.length; i++){
            require(_validSigners[i] != address(0), "zero address detected");
        ...
        noOfValidSigners = uint8(_validSigners.length);
        require(newQuorum <= noOfValidSigners, "quorum greater than valid signers");
        quorum = newQuorum;
